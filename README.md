...
프리미엄 보드게임 스타일의 요트다이스 웹 게임입니다. 설치와 회원가입 없이 브라우저에서 바로 실행되며, 한 기기 로컬 플레이와 2~7인 실시간 온라인 플레이를 지원합니다.
...
- **로컬 1~7인:** 한 기기에서 차례대로 플레이
- **온라인 2~7인:** 방장이 6자리 방 코드 또는 초대 링크를 공유하고 각자 기기에서 접속

## 온라인 플레이 방법

1. `새 게임` → `온라인 2~7인 플레이`를 선택합니다.
...
- 실제 6면 CSS 3D 주사위, 관성 회전·개별 착지·바닥 그림자 애니메이션
- 움직임 최소화 설정(`prefers-reduced-motion`) 지원
- 12개 요트다이스 점수 카테고리
- 상단 합계 63점 이상 보너스 +35점
- 예상 점수 및 추천 족보
- 상대 차례에도 내 점수표 및 모든 참가자의 점수표 열람
- 점수 확정 시 선택한 족보와 점수를 전체 화면에 동기 강조
- 같은 방에서 여러 게임을 이어가며 순위 포인트 누적 리더보드 제공
- 한 게임의 순위 포인트는 참가 인원수부터 1점씩 차감(7명: 1위 7P~7위 1P, 동점은 같은 포인트)
