...
function renderDice(animate=false){const enabled=canAct()&&state.rolls>0;$('#diceRow').innerHTML=state.dice.map((n,i)=>`<div class="die-wrap"><button class="die ${state.held[i]?'held':''} ${animate&&!state.held[i]?'rolling':''}" data-i="${i}" aria-label="주사위 ${n}${state.held[i]?', 고정됨':''}" ${enabled?'':'disabled'}>${pips(n)}</button><span class="hold-label">${state.held[i]?'HOLD':' '}</span></div>`).join('');$$('.die').forEach(x=>x.onclick=()=>toggleHold(+x.dataset.i))}
...
function performRoll(sync){state.rolling=true;tone(180,.08,'sawtooth');if(sync)broadcastState();render();let ticks=0;const final=state.dice.map((n,i)=>state.held[i]?n:randomDie());const timer=setInterval(()=>{state.dice=state.dice.map((n,i)=>state.held[i]?n:randomDie());renderDice(true);if(++ticks>=7){clearInterval(timer);state.dice=final;state.rolls++;state.rolling=false;render();tone(280,.05);setTimeout(()=>tone(420,.06),55);if(sync)broadcastState()}},70)}
